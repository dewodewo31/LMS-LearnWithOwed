  ...
  return (
    <div className="grid gap-[18px] sm:grid-cols-2 lg:grid-cols-4" aria-hidden="true">
      {Array.from({ length: 4 }).map((_, i) => (
        <div key={i} className="animate-pulse overflow-hidden rounded-2xl border border-lp-border bg-lp-card">
          ...
          <div className="space-y-3 p-6">
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              <div className="h-9 w-28 rounded-lg bg-lp-card-hover" />
            </div>
          </div>
        </div>
      ))}
...
export default function ModulesSection() {
  ...
  return (
    <section id="modul" className="bg-lp-bg-soft px-4 py-[120px] sm:px-6 max-[767px]:py-[80px]">
      <div className="lp-container">
        <header className="mb-14 lp-reveal">
        ...
        {isLoading ? (
          ...
          <div role="alert" className="mx-auto max-w-md rounded-2xl border border-[#EF4444]/25 bg-[#EF4444]/10 px-6 py-8 text-center">
            ...
            <button
              onClick={refetch}
              className="mt-4 inline-flex items-center gap-2 rounded-lg border border-[#F87171]/40 px-5 py-2 text-xs font-bold text-[#F87171] transition-colors hover:bg-[#EF4444]/20"
            >
              Coba Lagi
            </button>
          </div>
          ...
          <div className="mx-auto max-w-md rounded-2xl border border-dashed border-lp-border px-6 py-14 text-center">
            <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-lg bg-lp-accent-soft text-lp-accent">
          ...
          <>
            ...
            <div className="mt-14 text-center">
              <Link
                to="/modules"
                className="inline-flex items-center rounded-lg border border-lp-border bg-lp-card px-8 py-3.5 text-sm font-semibold text-lp-text transition-colors hover:bg-lp-accent-soft"
              >
                Lihat Semua Modul
              </Link>
            </div>
